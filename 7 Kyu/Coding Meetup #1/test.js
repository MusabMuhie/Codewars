...
describe("Fixed tests", () => {
  ...
  it("should find JavaScript developers from Europe", () => {
    assert.strictEqual(countDevelopers(list1.slice()), 1, `List of developers: ${JSON.stringify(list1)}`);
  });
  ...
  it("should not find HTML developers from Europe", () => {
    assert.strictEqual(countDevelopers(list2.slice()), 0, `List of developers: ${JSON.stringify(list2)}`);
  });
});
...
// Working solution
function countDevelopersSolution(list) {
  return list
    .reduce((accu, curr) => (curr.language === 'JavaScript' &&
                             curr.continent === 'Europe') ? accu + 1 : accu, 0);
}

  it("should work for random developers", () => {
    for (let i = 0; i < 20; i++) {
      let listFullCopy = listFull.slice();
      shuffleArray(listFullCopy);
      // take some of the developers
      listFullCopy = listFullCopy.slice(0, Math.floor(Math.random() * 10) + 5);
  
      assert.strictEqual(countDevelopers(listFullCopy.slice()), countDevelopersSolution(listFullCopy), `List of developers: ${JSON.stringify(listFullCopy)}`);
    }
  });
});