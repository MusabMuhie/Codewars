...
function scoreboard(whoAteWhat){
    ...
    whoAteWhat.forEach(el=>{
      let score=(el.chickenwings*5)+(el.hamburgers*3)+(el.hotdogs*2)
      res.push({name:el.name, score:score})
    })
    res.sort((a,b)=>b.score==a.score? a['name'].localeCompare(b['name']) : b.score-a.score)
    return res
  }