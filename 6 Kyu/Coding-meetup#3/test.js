describe("Tests", () => {
  ...
  var possibleLanguages = ['Python', 'Ruby', 'JavaScript'];
  ...
  for (let i = 0; i < listFull.length; i++) {
    listFull[i].language = possibleLanguages[makeRandomNumber(0,possibleLanguages.length-1)];
  }
  ...
  // Working solution
  function isLanguageDiverseSolution(list) {
    const languages = { Python: 0, Ruby: 0, JavaScript: 0 };
    list.forEach(dev => languages[dev.language]++);
    
    const languageCount = Object.keys(languages).map(key => languages[key]);
    return (Math.max(...languageCount) / Math.min(...languageCount) <= 2);
  }
  
  const developersAlwaysPresent = [
    { firstName: 'Noel', lastName: 'O.', country: 'Albania', continent: 'Europe', age: 23, language: 'Python' },
    { firstName: 'Fatima', lastName: 'A.', country: 'Algeria', continent: 'Africa', age: 25, language: 'JavaScript' },
    { firstName: 'Laia', lastName: 'P.', country: 'Andorra', continent: 'Europe', age: 55, language: 'Ruby' },
  ];
  ...
  (function() {
    for (let i = 0; i < 50; i++) {
      ...
      // Shorten the array and add developers always present
      listFullCopy = listFullCopy.slice(0,20).concat(developersAlwaysPresent);
      
      // Log the input for debugging purposes
      console.log(`Random test #${i+1}`);
      console.log(`List of developers: ${JSON.stringify(listFullCopy)}`);
      
      let expected = isLanguageDiverseSolution(listFullCopy);
      let actual = isLanguageDiverse(listFullCopy);
